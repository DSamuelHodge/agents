...
${prompt}` : prompt;
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        const isRetryable = errorMsg.includes("rate limit") || errorMsg.includes("timeout") || errorMsg.includes("503") || errorMsg.includes("429");
...
__name(getRoleById, "getRoleById");

// worker/src/workflow.ts
var MAX_OUTPUT_SIZE = 32 * 1024;
var MAX_CONTEXT_SIZE = 128 * 1024;
...
... [Output truncated. Original size: ${output.length} chars, showing first ${MAX_OUTPUT_SIZE} chars]`;
  ...
  async runWorkflow(featureRequest) {
    ...
    for (const roleId of WORKFLOW_SEQUENCE) {
      ...
      try {
        const contextStr = Object.entries(context.outputs).map(([key, value]) => `
### ${key} Output:
${value}`).join("\n");
        const fullInput = contextStr ? `${featureRequest}

Previous Agent Outputs:${contextStr}` : featureRequest;
  ...
  async runAgentChat(roleId, message, context) {
    const role = getRoleById(roleId);
    if (!role) {
...
// worker/src/utils/responses.ts
function jsonSuccess(data, metadata) {
  const body = { ok: true, data, metadata };
...
function jsonError(message, status = 500, details, code) {
  const body = { ok: false, message, details, code };
  return new Response(JSON.stringify(body), {
    status,
    headers: { "Content-Type": "application/json" }
  });
...
function validateRequestSize(request, maxBytes = 256 * 1024) {
...
function summarizeIfNeeded(text, maxChars = 8e3) {
...
__name(summarizeIfNeeded, "summarizeIfNeeded");

// worker/src/index.ts
var WorkflowCoordinator = class {
  static {
    __name(this, "WorkflowCoordinator");
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  state;
  // DurableObjectState from @cloudflare/workers-types
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor(state) {
    this.state = state;
  }
...
var src_default = {
  async fetch(request, env) {
    ...
    if (path2 === "/workflow" && request.method === "POST") {
      try {
    ...
    if (path2.startsWith("/agent/") && path2.endsWith("/chat") && request.method === "POST") {
      try {
        const roleId = path2.split("/")[2];
        const role = getRoleById(roleId);
        if (!role) {
    ...
    return jsonError("Not found", 404, `Path ${path2} not recognized`);
  }
};
...
// .wrangler/tmp/bundle-HsjZKd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HsjZKd/middleware-loader.entry.ts
